...
이름은 bash 판이 쓰던 것 그대로다. 병행 기간에는 두 구현이 같은 `.last-check` 로
스로틀을 걸고 같은 `.lock` 으로 직렬화해야 했기 때문인데, bash 가 사라진 뒤에도
바꿀 이유가 없다 — **이미 배포된 기기의 디스크에 그 이름으로 상태가 있다.** 이름을
바꾸면 오류가 나는 게 아니라 상태가 조용히 두 벌 생기고, 그건 나중에 "쿨다운이
리셋됐다" 나 "rotate 가 두 번 돌았다" 로만 관찰된다.
...
# bash 의 `codex_account_*_path` 가 만들던 이름 그대로. 상수로 뽑아 두는 것은 테스트가
# 경로 조립이 아니라 **이름 자체**를 단언할 수 있게 하기 위해서다.
LOG_NAME = "rotate.log"
...
def lock_path(settings: Settings) -> Path:
    ...
    병행 기간에는 bash 와 같은 프로토콜이어야 해서 `fcntl` 이나 owner 파일을 넣지
    못했다. 그 제약은 풀렸지만 교체가 공짜는 아니다 — 배포된 기기에 이 이름의 락이
    남아 있을 수 있고, 프로토콜을 바꾸면 옛 락을 든 프로세스와 새 프로세스가 서로를
    못 본다. 바꾼다면 그 전이까지 설계해야 한다.

    이 경로가 디렉토리가 아닌 경우(0바이트 파일·깨진 심링크)는 bash 에서 stale 판정이
    아예 돌지 않아 영구 교착이었다 — `store` 가 그것을 세 번째 갈래로 잡아 보고한다
    (§7.5 D2).
    """
