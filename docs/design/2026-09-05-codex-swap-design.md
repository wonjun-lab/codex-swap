...
| 진입점 | `codex-swap` **하나만** | 병행 기간에는 `uv tool install` 이 dotfiles 소유의 `~/.local/bin/codex-account` 심링크와 충돌해 설치 자체가 실패했다. 그 심링크가 없어진 지금은 호환 이름을 가져올 수 있지만, 두 이름을 두어 얻을 것이 없다 |
...
> 절들이라 근거가 사라졌다 — 진입점 이름 충돌(`uv tool install` 이 dotfiles 소유의
