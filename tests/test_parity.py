...
KNOWN_ASYMMETRY = {
    ...
    "exec": (
        "PATH 에 놓인 wrapper 가 부르는 자리다. 사람이 치는 명령이 아니라 `codex` 자체를 "
        "대신하는 것이라, 화면에 넣으면 화면 안에서 codex 를 띄우게 된다"
    ),
    "init": (
        "셸 프로필에 한 줄을 넣으라는 안내가 전부다. curses 안에서는 그 줄을 복사할 수도, "
        "새 셸을 열 수도 없어서 화면으로 옮기면 오히려 막다른 길이 된다. 화면 쪽에서 "
        "환경이 어긋난 것을 알리는 몫은 `doctor` 가 진다"
    ),
}
