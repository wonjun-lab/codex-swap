...
케이스 이름은 폐기된 bash `tests/test-codex-account.sh` 의 것을 그대로 가져왔다. 그
파일이 사실상의 실행 가능한 명세였고, 1:1 대응을 유지한 덕에 이관 중에는 차등 하니스의
픽스처로 쓸 수 있었다. bash 가 사라진 지금은 이 파일이 정책의 명세다.
