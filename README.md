...
bash 판은 481 줄의 정책 엔진을 담고 있는데, 그 핵심인 사다리 판정이 프로브·파일 I/O 와
엉켜 있어 격리 테스트가 되지 않는다. Python 판은 정책을 순수 함수로 떼어내 네트워크 없이
전 조합을 검증한다.

부수 효과로 외부 의존이 사라진다. bash 판은 `jq` · `base64` · `node` 를 요구했다.
...
bash 판과 **같은 파일을 같은 자리에** 쓴다. 그래서 둘을 나란히 두고 옮겨갈 수 있다.
